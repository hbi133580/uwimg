    ...
    for (int ch = 0; ch < im.c; ch++) {
        for (int y = 0; y < im.h; y++) {
            for (int x = 0; x < im.w; x++) {
                // Use method from summed area table wiki
                // summed area table val = a + b + c + d
                float a = get_pixel(im, x, y, ch);
                float b = get_pixel(integ, x, y - 1, ch);
                float c = get_pixel(integ, x - 1, y, ch);
                float d = get_pixel(integ, x - 1, y - 1, ch);
                ...
                set_pixel(integ, x, y, ch, a + b + c - d);
            }
        }
    }
    return integ;
...
image box_filter_image(image im, int s) {
    ...
    for (int ch = 0; ch < im.c; ch++) {
        for (int y = 0; y < im.h; y++) {
            for (int x = 0; x < im.w; x++) {
                float a = get_pixel(integ, x - padding - 1, y - padding - 1, ch);
                float b = get_pixel(integ, x + padding, y - padding - 1, ch);
                float c = get_pixel(integ, x - padding - 1, y + padding, ch);
                float d = get_pixel(integ, x + padding, y + padding, ch);
                ...
                set_pixel(S, x, y, ch, (d + a - b - c) / (s * s));
            }
        }
    }
    free_image(integ);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    image im_ix = convolve_image(im, gx_filter, 0);
    image im_iy = convolve_image(im, gy_filter, 0);
    ...
    for (int i = 0; i < im.h * im.w; i++) {
        float ix = im_ix.data[i];
        float iy = im_iy.data[i];
        float it = im.data[i] - prev.data[i];

        S.data[i] = ix * ix;
        S.data[i + im.h * im.w] = iy * iy;
        S.data[i + 2 * im.h * im.w] = ix * iy;
        S.data[i + 3 * im.h * im.w] = ix * it;
        S.data[i + 4 * im.h * im.w] = iy * it;
    }

    image smoothed_S = box_filter_image(S, s);
